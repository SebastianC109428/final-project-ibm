...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown', options = sites, value = 'All Sites', placeholder = "Select a Launch Site here", searchable = True),
                                html.Br(),
...
def get_scatter_chart(entered_site_1, entered_site_2):
    filtered_df = spacex_df
    if entered_site_1 == 'All Sites':
        df = filtered_df
        df_2 = df[df["Payload Mass (kg)"] >= entered_site_2[0]]
        df_3 = df_2[df["Payload Mass (kg)"] <= entered_site_2[1]]
        fig_2 = px.scatter(df_3, 
                           y="class", 
                           x="Payload Mass (kg)", 
                           color="Booster Version Category")
    else:
        df = filtered_df[filtered_df["Launch Site"] == entered_site_1]
        df_2 = df[df["Payload Mass (kg)"] >= entered_site_2[0]]
        df_3 = df_2[df["Payload Mass (kg)"] <= entered_site_2[1]]
        fig_2 = px.scatter(df_3, 
                           y="class", 
